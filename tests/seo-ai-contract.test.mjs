...
test("SEO generation uses bounded active fast model and persisted fallback",()=>{assert.match(seo,/@cf\/meta\/llama-3\.1-8b-instruct-fast/);assert.match(seo,/slice\(0,60\)/);assert.match(seo,/slice\(0,160\)/);assert.match(blog,/seo_description\|\|p\.excerpt/);});
