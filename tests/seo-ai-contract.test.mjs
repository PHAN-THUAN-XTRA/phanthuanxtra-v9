...
test("SEO generation uses bounded active fast model and persisted fallback",()=>{assert.match(seo,/@cf\/meta\/llama-3\.1-8b-instruct-fast/);assert.ok(seo.includes("clean(x.title,60)"));assert.ok(seo.includes("clean(x.description,160)"));assert.match(blog,/seo_description\|\|p\.excerpt/);});
