...
async function uploadWorker(assetJwt) {
  ...
  const metadata = {
    ...
    assets: { jwt: assetJwt, config: { not_found_handling: "404-page", run_worker_first: ["/", "/style.css", "/admin", "/admin/", "/admin.html", "/admin-control", "/admin-control.html", "/admin-recovery*", "/api/*", "/media/*"] } },
    triggers: { crons: ["*/5 * * * *"] },
